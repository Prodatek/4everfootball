...
| `POST /competitions/:competitionId/registrations/checkout` | Any authenticated → **assertCanManage(org)** | `amountKobo` always summed server-side from stored `priceKobo` snapshots. `payerEmail` now optional (found during Phase B live verification — was required, but `BANK_TRANSFER`/`CASH` never actually use it), falls back to the current user's account email |
