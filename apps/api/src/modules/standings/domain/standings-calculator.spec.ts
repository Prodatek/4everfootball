...
describe('computeStandings', () => {
  it('includes every entered team even with zero fixtures played', () => {
    ...
    expect(table.every((row) => row.played === 0 && row.points === 0)).toBe(
      true,
    );
  });
  ...
  it('breaks a full tie (points, goal difference, goals for) alphabetically by name', () => {
    ...
    // Both Alpha and Bravo drew once against Charlie: identical points/GD/GF,
    // so their relative order must fall back to alphabetical name.
    const alpha = table.find((row) => row.teamId === 'a')!;
    const bravo = table.find((row) => row.teamId === 'b')!;
    ...
    expect(alpha.position).toBeLessThan(bravo.position);

    // Charlie drew both its games, so it tops the table on points alone.
    expect(table[0].teamName).toBe('Charlie FC');
  });
